...
def initialize_matrix(u,r,R):
    M = [[None for i in range (len(u)+2)] for j in range (len(r)+2)]
    ...
    for y in range(len(r)+2):
        for x in range(len(u)+2):
            if y < 2 and x < 2:
                ...
                M[y][x] = r[y-2] # Set column labels
            elif y == 0:
                M[y][x] = u[x-2] # Set row labels
            elif x == 1:
                M[y][x] = M[y-1][x] + R[M[y][0]]['-']   # Set first column values
                #M[y][x] = M[y-1][x] + R['-'][M[y][0]]   # 2
            elif y == 1:
                M[y][x] = M[y][x-1] + R['-'][M[0][x]]   # Set first row values
                #M[y][x] = M[y][x-1] + R[M[0][x]]['-']   # 2
            else:
    ...
    for y in range(len(r)+2):
        for x in range(len(u)+2):
            if y < 2 or x < 2: # Ignore first 2 rows and columns
                continue
            #difference = R[M[y][0]][M[0][x]]
            difference = R[M[0][x]][M[y][0]]
            current = M[y][x]
            above = M[y-1][x]
            left = M[y][x-1]
            diagonal = M[y-1][x-1]
            if not difference:
                ...
                minimum = min(above+R[M[0][x]]['-'], left+R['-'][M[y][0]], diagonal+difference)
                #minimum = min(above+R['-'][M[0][x]], left+R[M[y][0]]['-'], diagonal+difference)
                M[y][x] = minimum
...
class MinDifferenceTest(unittest.TestCase):
    ...
    def test_align_unweighted(self):
        ...
        self.assertEqual(r, 'exponen-ial')
    ''''''
    def test_ejodl(self):
